  ...
  async function loadContent(key = adminKey) {
    ...
    try {
      ...
      if (res.status === 401) {
        ...
        throw new Error(
          `Login failed — sent: ${
            debug.receivedLength ?? "?"
          } characters, expected: ${
            debug.expectedLength ?? "?"
          }, header received: ${
            debug.receivedKeyPresent ? "yes" : "no"
          }, match: ${
            debug.matches ? "yes" : "no"
          }`
        );
      }
